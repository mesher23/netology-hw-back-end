<?php 
    ...
    if ( empty($_SESSION['log_user']) )
    {
        ...
        redirect('u/meshcheryakov/back/2.4/index.php');
    }
...
<head>
    ...
    <link href="/u/meshcheryakov/back/2.4/css/style.css" rel="stylesheet">
...
<div class="form">
        ...
        <form enctype="multipart/form-data" action="user_form.php" method="POST">
            ...
            <?php 
            if ( $number_post === null ):
                $errors [] = 'Число не введенно';
            elseif ($number_post > 5):
                echo $errors[] = 'Количестов пунтков должно быть меньше 5';
            elseif ($number_post < 1):
                echo $errors[] = 'Количестов пунтков не может быть меньше 1';
            else:
            ?>
            ...
            if ( count($_POST) > 1 ) 
            {
                foreach( $_POST as $data_id => $value ) 
                {
                    ...
                    {
                        echo $errors [] = 'Не все пункты были отмечены';
                        die;
                    } elseif ( $value === '' )
                    {
                        echo $errors [] = 'Не все пункты были заполнены';
                        die;
                    } elseif ( file_exists("./json/".md5($_POST["name"]).".json") )
                    {
                        echo $errors [] = 'Тест с таким именем уже есть! Придумаёте другое имя.';
                        die;
                    } else
                    {
